...
const HeroSection = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const scaleValue = 1 + scrollY * 0.002; // Adjust the speed of scaling
      const opacityValue = Math.max(1 - scrollY * 0.002, 0); // Fade out as you scroll

      // Apply scaling only to the hero image
    ...
    <section className="relative w-full min-h-screen bg-gradient-to-br from-[#000000] to-[#1C1C1C] text-white flex justify-center items-center overflow-x-hidden">
      ...
      <div className="w-full overflow-x-hidden max-w-7xl mx-auto px-4 sm:px-6 py-20 md:py-24 grid grid-cols-1 md:grid-cols-2 gap-10 items-center relative z-10 text-center md:text-left">
        ...
        <motion.div
          ...
          className="relative flex justify-center md:justify-end hero-img"
        >
          ...
          <motion.div
            ...
            className="absolute w-[400px] h-[400px] bg-gradient-to-tr from-[#FF4C4C]/30 via-[#F4D03F]/20 to-transparent rounded-full blur-2xl z-0 md:w-[400px] md:h-[400px] lg:w-[500px] lg:h-[500px]"
          />

          {/* Hero Image with Zoom on Scroll */}
          <motion.img
            src={HeroImg}
            alt="Hero Logo"
            className="relative z-10 max-w-full h-auto drop-shadow-[0_10px_25px_rgba(0,0,0,0.4)] rounded-xl transition-transform duration-300"
